...
local symbols = {"hbar", "ell", "nabla", "infty", "dots", "leftrightarrow", "mapsto", "setminus", "mid", "bigcap", "bigcup", "cap", "cup", "land", "lor", "subseteq", "subset", "implies", "impliedby", "iff", "exists", "forall", "equiv", "square", "neq", "geq", "leq", "gg", "ll", "sim", "simeq", "approx", "propto", "cdot", "oplus", "otimes", "times", "star", "perp", "det", "exp", "ln", "log", "partial",}


local function contains(collection, word)
    for _, char in ipairs(collection) do
        if char == word then
            return true
        end
    end
...
local function isGreekChar(word)
   return contains(greek_alphabet, word)
end

local function isSymbol(word)
    return contains(symbols, word)
end
